...
reduce_cluster_group <- function(dataframe, cluster_df, summary_func = mean) {
  ...
  common_regions <- intersect(rownames(dataframe), names(cluster_df))

  if(length(common_regions) == 0) {
    stop("No matching regions between dataframe and cluster_df")
  }

  filtered_df <- dataframe[common_regions, ]
  filtered_cluster <- cluster_df[common_regions]
